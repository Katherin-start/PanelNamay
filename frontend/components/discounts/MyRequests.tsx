...
export default function MyRequests() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-start justify-between gap-4 pb-5 border-b border-gray-100 dark:border-gray-700">
        <div>
          <p className="eyebrow">Solicitudes</p>
          <h3 className="mt-2 text-xl font-semibold text-namay-navy dark:text-gray-100">Mis solicitudes de descuento</h3>
          <p className="mt-1 text-sm text-namay-steel/60 dark:text-gray-400 font-medium">Revisa el estado de las solicitudes enviadas y el historial de creación.</p>
        </div>
      ...
      {requests.length === 0 ? (
        <div className="bg-namay-cream/40 dark:bg-gray-800/40 border border-dashed border-gray-200 dark:border-gray-700 p-10 text-center">
          ...
          <p className="text-sm text-namay-steel/60 dark:text-gray-400 font-medium">No tienes solicitudes registradas todavía.</p>
        </div>
      ) : (
            ...
            <div key={r.id} className="bg-white dark:bg-gray-800 p-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  <TagIcon className="h-5 w-5 text-namay-steel/60 dark:text-gray-400 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
                  <div className="min-w-0 flex-1">
                    <p className="text-base font-semibold text-namay-navy dark:text-gray-100">{r.nombre}</p>
                    <p className="text-sm text-namay-steel/70 dark:text-gray-400 font-medium mt-0.5">{r.descripcion}</p>
                  </div>
                </div>
                <div className="flex flex-col items-start gap-1 sm:items-end flex-shrink-0">
                  ...
                  <span className="text-[11px] text-namay-steel/60 dark:text-gray-400 font-medium tabular">
                    {r.creado_en ? new Date(r.creado_en || r.solicitado_en).toLocaleDateString('es-PE') : ''}
                  </span>
                </div>
              </div>

              <div className="mt-5 grid gap-px sm:grid-cols-3 bg-gray-100 dark:bg-gray-700">
                <div className="bg-white dark:bg-gray-800 px-3 py-2.5">
                  <p className="eyebrow">Valor</p>
                  <p className="text-sm font-semibold text-namay-navy dark:text-gray-100 mt-1 tabular">
                ...
                <div className="bg-white dark:bg-gray-800 px-3 py-2.5">
                  ...
                  <p className="text-sm font-semibold text-namay-navy dark:text-gray-100 mt-1 uppercase tracking-wide">
                    {r.aplica_a}
                  </p>
                </div>
              </div>
