...
export default function AdminRequests() {
  ...
  if (user.rol !== 'ADMINISTRADOR')
    return (
      <div className="text-center py-20 text-sm text-namay-steel/60 font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-100">
        <div className="bg-white p-6">
          <div className="flex items-center gap-2">
            <ClockIcon className="h-4 w-4 text-namay-steel/50" strokeWidth={1.5} />
            <p className="eyebrow">Pendientes</p>
          </div>
          <p className="mt-3 text-3xl font-medium text-namay-navy tabular">{pendingRequests.length}</p>
          <p className="text-[11px] mt-1.5 text-namay-steel/60 font-medium">Solicitudes activas</p>
        </div>
        <div className="bg-white p-6">
          <div className="flex items-center gap-2">
          ...
          <p className="text-[11px] mt-1.5 text-namay-steel/60 font-medium">Este mes</p>
        </div>
        <div className="bg-white p-6">
          <div className="flex items-center gap-2">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between pb-4 border-b border-gray-100">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold text-namay-navy">Todas las solicitudes</h2>
            <p className="text-sm text-namay-steel/60 font-medium mt-0.5">
        ...
        {requests.length === 0 ? (
          <div className="bg-namay-cream/40 border border-dashed border-gray-200 p-10 text-center">
            ...
            <p className="text-sm text-namay-steel/60 font-medium">No hay solicitudes registradas</p>
          </div>
        ) : (
              ...
              <div key={r.id} className="bg-white p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-base font-semibold text-namay-navy">{r.nombre}</h3>
                    <p className="text-sm text-namay-steel/70 font-medium mt-1">{r.descripcion}</p>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-px bg-gray-100">
                  <div className="bg-white px-4 py-3">
                    <p className="eyebrow">Valor del descuento</p>
                    <p className="text-base font-semibold text-namay-navy mt-1.5 tabular">
                  ...
                  <div className="bg-white px-4 py-3">
                    ...
                    <p className="text-base font-semibold text-namay-navy mt-1.5 uppercase tracking-wide">
                      {r.aplica_a}
                    </p>
                  </div>
                </div>
                ...
                {r.estado === 'pendiente' ? (
                  ...
                  <div className="flex justify-end gap-3 mt-5 pt-5 border-t border-gray-100">
                    <span className="text-[11px] font-semibold uppercase tracking-[0.2em] text-namay-steel/40">No se puede cambiar</span>
                  </div>
                )}
              </div>
            ))}
